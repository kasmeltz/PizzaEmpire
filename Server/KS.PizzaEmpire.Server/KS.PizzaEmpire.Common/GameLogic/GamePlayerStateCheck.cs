namespace KS.PizzaEmpire.Common.GameLogic
{
    ...
    public class GamePlayerStateCheck : IResetable
    {
        ...
        public List<ItemQuantity> ItemQuantityGreaterThan { get; set; }
        ...
        /// <summary>
        /// Checks whether the player has more than the
        /// indicated number of items
        /// </summary>
        /// <param name="player">The player to check.</param>
        public bool CheckItemsGreaterThan(GamePlayer player)
        {
            if (ItemQuantityGreaterThan == null ||
                ItemQuantityGreaterThan.Count == 0)
            {
                return true;
            }

            for (int i = 0; i < ItemQuantityGreaterThan.Count; i++)
            {
                ItemQuantity checkIQ = ItemQuantityGreaterThan[i];
                if (!player.BuildableItems.ContainsKey(checkIQ.ItemCode) ||
                    player.BuildableItems[checkIQ.ItemCode] <= checkIQ.Quantity)
                {
                    return false;
        ...
        public bool CheckAll(GamePlayer player)
        {
            ...
            if (!CheckItemsGreaterThan(player))
            {
                return false;
            }
        ...
        public void Reset()
        {
            ItemQuantityGreaterThan.Clear();
            ItemQuantityLessThan.Clear();
            WorkItemsInProgress.Clear();
            RequiredLevel = null;
            CanBuildItem = BuildableItemEnum.None;
            Coins = null;
